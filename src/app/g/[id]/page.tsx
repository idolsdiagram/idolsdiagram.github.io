...
export default async function Page({ params }: { params: { id: string } }) {
    ...
    const onemanJson: OnemanJson = {} as OnemanJson
    // 手入力分を取得
    ...
    const onemanJson2: OnemanJson = {} as OnemanJson
    ...
    // 手入力分を追加
    if (onemanJson?.values && onemanJson2?.values) {
        onemanJson.values = onemanJson.values.concat(onemanJson2.values)
        events = onemanJson.values.slice(1).filter((value) => {
            return value[7] === 'OK'
        }).filter((value) => {
